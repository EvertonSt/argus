...
import { renderPrComment, type CiReportInput } from './ci-report.js';
...
program
  ...
  .action(() => {
    ...
    for (const entry of entries) {
      ...
      process.stdout.write(`  ${verdictBadge(entry.verdict)} ${paint('dim', `(${confidence})`)}  ${entry.testTitle}\n`);
      process.stdout.write(`    ${paint('gray', entry.reasoning)}\n`);
      if (entry.suggestedFix) {
        process.stdout.write(`    ${paint('blue', 'suggested fix:')} ${paint('gray', entry.suggestedFix)}\n`);
      }
      process.stdout.write(`    ${paint('gray', `error: ${entry.errorMessage.split('\n')[0]}`)}\n\n`);
    }
