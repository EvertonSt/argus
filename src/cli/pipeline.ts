...
export async function runPipeline(options: RunOptions): Promise<RunOutcome> {
  ...
  if (!mock) {
    ...
    log.info(
      `Provider: ${config.aiProvider} (${providerLabel}). ` +
        `Budget: ${config.maxAiCalls} calls total ` +
        `(ARGUS_MAX_AI_CALLS).`,
    );
  }
  ...
  const blocking = fresh.filter((bug) =>
    meetsThreshold(bug.severity, config.severityFailThreshold),
  );
  ...
  log.success(
    `Artifacts written to ${path.relative(config.paths.root, runDir).replace(/\\/g, '/')}`,
  );
  log.blank();
  log.table([
...
export function assertRunnable(config: ArgusConfig, mock: boolean): void {
  if (mock || config.aiProvider === 'mock') return;
  ...
  if (
    (config.aiProvider === 'openai-compatible' || config.aiProvider === 'ollama') &&
    config.openaiCompatible.requireApiKey &&
    !config.openaiCompatible.apiKey
  ) {
    throw new ArgusError(
      `An API key is not set for provider "${config.aiProvider}".`,
