 ...
 */
const QUOTED = String.raw`["'\x60\x5c]((?:[^"'\x60\x5c]|\\.)*)["'\x60\x5c]`;
...
export const TEMPLATE_RULES: TemplateRule[] = [
  ...
  rule(
    'navigate-to-route',
    String.raw`\b(?:navigate|go)\s+to\s+["'\x60]?(\/[^\s"'\x60]*)["'\x60]?`,
    (m) => `await page.goto(${lit(m, 1)});`,
  ),
  rule(
    'on-page-noop',
  ...
  rule(
    'click-nth-selector',
    String.raw`\bclick\s+(?:on\s+)?the\s+(first|last|second)\s+(?:element\s+)?(?:matching\s+)?` +
      QUOTED,
  ...
  rule(
    'click-selector',
    String.raw`\bclick\s+(?:on\s+)?(?=["'\x60](?:#|\[))["'\x60\x5c]((?:[^"'\x60\x5c]|\\.)*)["'\x60\x5c]`,
    (m) => `await page.locator(${lit(m, 1)}).first().click();`,
  ),
  rule(
    'click-text',
    String.raw`\bclick\s+(?:on\s+)?` + QUOTED,
    (m) => `await page.getByText(${lit(m, 1)}).first().click();`,
  ),

  // --- checkboxes ---------------------------------------------------------
  rule(
    'check-nth',
    String.raw`\b(?:check|tick|toggle)\sthe\s+(first|last|second)\s+(?:element\s+)?(?:matching\s+)?` +
      QUOTED,
    (m) => `await ${ordinalLocator(m[1] as string, lit(m, 2))}.check();`,
  ),
  rule(
    'check-selector',
    String.raw`\b(?:check|tick|toggle)\s+` + QUOTED,
    (m) => `await page.locator(${lit(m, 1)}).first().check();`,
  ),
  ...
  rule(
    'assert-text',
    QUOTED + String.raw`\s+should\s+have\s+text\s+` + QUOTED,
    (m) => `await expect(page.locator(${lit(m, 1)})).toHaveText(${lit(m, 2)});`,
  ),
  rule(
    'assert-nth-checked',
  ...
  rule(
    'assert-not-checked',
    QUOTED + String.raw`\s+should\s+not\s+be\s+checked`,
    (m) => `await expect(page.locator(${lit(m, 1)}).first()).not.toBeChecked();`,
  ),
  rule(
    'assert-checked',
    QUOTED + String.raw`\s+should\s+be\s+checked`,
    (m) => `await expect(page.locator(${lit(m, 1)}).first()).toBeChecked();`,
  ),
  rule(
    'assert-not-visible',
  ...
  rule(
    'assert-visible',
    QUOTED + String.raw`\s+should\s+be\s+visible`,
    (m) => `await expect(page.locator(${lit(m, 1)}).first()).toBeVisible();`,
  ),
  rule(
    'assert-should-not-see',
    String.raw`\bshould\s+not\s+see\s+` + QUOTED,
    (m) => `await expect(page.getByText(${lit(m, 1)})).toHaveCount(0);`,
  ),
  rule(
    'assert-should-see',
    String.raw`\bshould\s+see\s+` + QUOTED,
    (m) => `await expect(page.getByText(${lit(m, 1)}).first()).toBeVisible();`,
  ),

  // --- security -----------------------------------------------------------
  rule(
    'reject-empty-input',
    String.raw`\b(?:reject|guard|block|ignore)(?:\s+against)?(?:\s+an)?\s+empty\s+(?:submission|input|name|task)\b`,
    () => `expect(input).toHaveValue();`,
  ),
  rule(
    'validate-email-format',
    String.raw`\b(?:validate|check)(?:\s+the)?\s+(?:that\s+(?:an?)\s+)?email\s+format\b`,
    /* eslint-disable-next-line no-useless-escape */
    () => `expect(page.getByLabelText(/email/i)).toHaveValue(/^[^\s@]+@[^\s@]+\.[^\s@]+$/);`,
  ),
  rule(
    'require-https',
    String.raw`\b(?:require|enforce|use)\s+https\b`,
    () => `// Page should be served over HTTPS\nexpect(page.url()).toMatch(/^https:/);`,
  ),

  // --- accessibility ------------------------------------------------------
  rule(
    'accessible-name',
    String.raw`\b(?:have|check(?:\s+for)?)\s+an\s+accessible\s+name\b`,
    () => `await expect(page.getByRole('button', { name: /.*/ })).toBeVisible();`,
  ),
  rule(
    'keyboard-navigable',
    String.raw`\b(?:be\s+)?keyboard\s+navigable\b`,
    () => `await page.keyboard.press('Tab');\nawait expect(page.locator(':focus')).toBeVisible();`,
  ),
  rule(
    'no-autoplay-media',
    String.raw`\b(?:no|prohibit)\s+autoplay(?:\s+media)?\b`,
    () =>
      `// Media must not autoplay\nconst video = page.locator('video');\nif (await video.count()) { await expect(video).toHaveAttribute('autoplay', null); }`,
  ),
  rule(
    'color-contrast',
    String.raw`\b(?:maintain|check)\s+(?:sufficient\s+)?color\s+contrast\b`,
    () =>
      `// Color contrast should be at least 4.5:1 (verify via axe-core)\nconst CONTRAST_MIN = 4.5;`,
  ),
];
