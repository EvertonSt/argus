...
const summary: RunSummary = {
  ...
  failures: [
    ...
    },
    {
      ...
      errorMessage: 'Error: expect(locator).toHaveText(expected) failed. Expected "3", received "4".',
    },
  ],
};
...
describe('validateTriageResponse', () => {
  ...
  it('rejects an unknown verdict', () => {
    const result = validateTriageResponse(JSON.stringify({ verdict: 'maybe', confidence: 0.5, reasoning: 'x' }), 'tc');
  ...
  it('rejects a missing reasoning', () => {
    const result = validateTriageResponse(JSON.stringify({ verdict: 'flaky', confidence: 0.5 }), 'tc');
    expect(result.ok).toBe(false);
  });
