...
describe('compareTwoStrings (inlined Dice coefficient)', () => {
  ...
  it('scores unrelated titles below the dedupe threshold', () => {
    expect(
      compareTwoStrings('Submitting an empty task is rejected', 'The stats page totals'),
    ).toBeLessThan(0.55);
  });
});
...
describe('checkDuplicate (ported string-similarity matching)', () => {
  ...
  it('sorts matches best-first', () => {
    const matches = checkDuplicate(
      'Deleting a task removes the wrong task from the list',
      existing,
    );
    if (matches.length > 1) {
  ...
  const triage: TriageResult[] = [
    {
      ...
      verdict: 'real_bug',
      confidence: 0.9,
      reasoning: 'wrong row removed',
    },
    {
      testCaseId: 'empty-task',
      verdict: 'real_bug',
      confidence: 0.8,
